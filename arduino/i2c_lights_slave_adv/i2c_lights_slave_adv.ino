...
void setup() {
	...
	patternPtrs[5] = frontUpperOn;
	patternPtrs[6] = backUpperOn;
	patternPtrs[7] = underglowRed;
	patternPtrs[8] = underglowBlue;
	patternPtrs[9] = underglowOff;
	patternPtrs[10] = idleOn;
	patternPtrs[11] = idleOff;
...
void idleOn(int index, byte state){
  ...
  uint8_t wave1 = beatsin8(1, 0, 255);                        // That's the same as beatsin8(9);
  ...
  for (int i=0; i<NUM_LEDS_UPPERS; i++) {
	...
	EVERY_N_SECONDS(3) {                                        // Change the target palette to a random one every 5 seconds.
...
void fadeAllStrips(int index, byte state){
	...
	if(i <= 128){
		...
		else{	//once the for loop condition is met change to the next state
...
void wave(int index, byte state){
	...
	for(int i = 0; i < NUM_LEDS_UNDERGLOW; i++){
		...
		if(sin8((i * 10) - 0.8 * pow(loopcount,translate_speed))>128){value = 1;}
		else{value = 0;}
	...
	loopcount++;
	FastLED.show();
	if(loopcount >= 200){patternEnabled[index] = false;}
}
